...
- `pure_usernotifications` becomes the default on macOS (the flag may be renamed or removed).

---

### Release strategy — 4.x+1 and 5.0-rc1 in parallel

The next release will be a 4.x minor that ships the `pure_usernotifications` integration as-is, including the intentional API break on `NotificationHandle::id()` — it now returns `NotificationId` instead of `u32`. This is a semver break that we are accepting early and keeping until 5.0, so users have something concrete to migrate against.
...
| `pure_usernotifications` feature flag                  | required to opt in to `UNUserNotificationCenter` | becomes the default (flag renamed or removed) |                                                                                                                   |
